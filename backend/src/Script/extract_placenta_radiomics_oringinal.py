...
def process_single_case(img_path, mask_path, args):
    ...
    if "_0000.nii.gz" in img_filename:
        case_id = img_filename.replace("_0000.nii.gz", "")
    else:
        case_id = img_filename.replace(".nii.gz", "")
    ...
    try:
        ...
        if target_voxels < 10:
            print(f"[Skipped][{case_id}] Insufficient target voxels ({target_voxels} < 10)")
            return None
        
        if image.GetSize() != mask.GetSize():
            print(f"[Skipped][{case_id}] Image-mask size mismatch (image: {image.GetSize()}, mask: {mask.GetSize()})")
            return None
        ...
        print(f"[Completed][{case_id}] Features: {len(feat_dict)-3} | Time: {feat_dict['ProcessTime']}s | Memory: +{mem_delta}MB")
        
        return feat_dict

    except Exception as e:
        print(f"[Error][{case_id}] Processing failed: {str(e)}")
        traceback.print_exc()
        return None
    finally:
        try:
...
def main():
    ...
    if all_features:
        ...
        cols = ["CaseID", "ProcessTime", "TargetVoxelCount"] + [
            col for col in df.columns if col not in ["CaseID", "ProcessTime", "TargetVoxelCount"]
        ]
        df = df[cols]
...
Result Dimensions:
  ...
  Number of features: {feature_dim} (including shape, first-order, and texture features)
